...
class ELDERY_RISK_RNN_Train(object):
    ...
    def __init__(self, train_config):
        ...
        self.rnn_net = ELDERY_RISK_RNN(net_config,training = True)
        self.predict = self.rnn_net.build_net_graph()

        #dot_img_file = '/tmp/model_1.png'
        #tf.keras.utils.plot_model(self.predict, to_file=dot_img_file, show_shapes=True)
    ...
    def train_rnn(self):
        ...
        dataset = DataLoad('./dataset/train/', time_step=self.rnn_net.time_step, class_num= self.rnn_net.class_num,sensor_columns = self.rnn_net.sensor_columns)
        saver = tf.train.Saver()
        #log head

        self.train_logger.info('Columns = %s',self.rnn_net.sensor_columns)
        self.train_logger.info('Sensors = %s',str(SensorTable))
        self.train_logger.info('Sample Rate = 50hz Time Step = %d', self.rnn_net.time_step)
        self.train_logger.info('DataSize = %d', len(dataset._all_data.acc_x.values))
