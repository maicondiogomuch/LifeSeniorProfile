...
class ELDERY_RISK_RNN(object):

    def __init__(self, net_config, test_batch_size=None, time_step=None, training=True):
        ...
        self.sensor_columns = list(map(int,net_config['sensor_columns']))
        self.training = training

        print('sensorcolumns',self.sensor_columns)
    ...
    def build_net_graph(self):
        ...
        print('btensor shape input',self.input_tensor.shape)

        #Create output layer
        input_x = tf.reshape(self.input_tensor, [-1, self.senor_data_num])

        #Dropout Layer
        print('treinamento = ',self.training)
        input_x = tf.compat.v1.layers.dropout(inputs = input_x,rate=0.1,training = self.training)
